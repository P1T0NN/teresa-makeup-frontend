 ...
 * `COMPANY_DATA.SALON_TIMEZONE`, regardless of the timezone the owner used when creating the event.
 *
...
function wallClockToSalonMs(iso: string): number | null {
	...
	return new CalendarDateTime(y, mo, d, h, mi, s).toDate(COMPANY_DATA.SALON_TIMEZONE).getTime();
}
...
function allDayBoundaryToMs(dateStr: string): number | null {
	...
	const [y, mo, d] = parts;
	return new CalendarDateTime(y, mo, d, 0, 0, 0).toDate(COMPANY_DATA.SALON_TIMEZONE).getTime();
}
